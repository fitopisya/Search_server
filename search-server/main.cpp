...
public:
 ...
 // universal top documents with 2 parameters, where document_predicate is a function with 3 parameters: document_id, document_data.status, document_data.rating
    ...
    vector<Document> FindTopDocuments(const string& cin_query) const {
        const Query query = ParseQuery(cin_query);
 ...
 // top documents with 1 parameter 
...
private:
    ...
    vector<Document> FindAllDocuments(const Query& query) const {
        ...
        for (const string& word : query.plus_words) {
            ...
            const double word_inverse_document_freq = WordInverseDocumentFreq(word);
