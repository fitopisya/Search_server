...
void SearchServer::AddDocument(int document_id, const std::string& document, DocumentStatus status, const std::vector<int>& ratings) {
    if (document_id < 0) {
        throw std::invalid_argument("Document id can not be less than 0"s);
    }
    ...
    for (auto& word : words) {
        document_to_word_freqs_[document_id][word] += 1.0 / static_cast<double>(words.size());
    }
...
void SearchServer::RemoveDocument(int document_id) {
    if (document_to_word_freqs_.count(document_id)) {
        std::map<std::string, double> search_doc = document_to_word_freqs_[document_id];
        std::string map_key;
        for (auto [key, value] : search_doc) {
            word_to_document_freqs_[key].erase(document_id);
            map_key = key;
        }
        word_to_document_freqs_.erase(map_key);
...
SearchServer::QueryWord SearchServer::ParseQueryWord(std::string text) const {
    ...
    if (text.empty() || text[0] == '-' || !IsValidWord(text)) {
        throw std::invalid_argument("The query is invalid"s);
    }
