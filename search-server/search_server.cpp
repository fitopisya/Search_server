...
void SearchServer::AddDocument(int document_id, std::string_view document, DocumentStatus status, const std::vector<int>& ratings) {
    if (document_id < 0) {
        throw std::invalid_argument("Document has negative id"s);
    }
    if (documents_.count(document_id)) {
        throw std::invalid_argument("Document with the same id already exist"s);
    }
    if (!IsValidWord(document)) {
        throw std::invalid_argument("Invalid char(s) in document"s);
    }
...
std::vector<std::string_view> SearchServer::SplitIntoWordsNoStop(std::string_view text) const {
    ...
    for (const auto& word : SplitIntoWordsView(text)) {
        if (!IsValidWord(word)) {
            throw std::invalid_argument("Invalid char(s)"s);
        }
        if (!IsStopWord(word)) {
...
SearchServer::QueryWord SearchServer::ParseQueryWord(std::string_view text) const {
    ...
    if (text.empty() || text[0] == '-' || !IsValidWord(text)) {
        throw std::invalid_argument("More than one - before word"s);
    }
